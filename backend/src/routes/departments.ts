/**
 * Departments CRUD (hierarchical).
 *   GET    /api/departments                view (filters: branchId, parentId, isActive)
 *   POST   /api/departments                create
 *   GET    /api/departments/:id            view
 *   PUT    /api/departments/:id            edit
 *   DELETE /api/departments/:id            delete (hard; blocked when children/users exist)
 */
import { Router } from 'express';
import { z } from 'zod';
import { requireInternal } from '../middleware/auth';
import { requirePermission } from '../middleware/rbac';
import { sendSuccess } from '../utils/response';
import { parseBody, parseParams, parseQuery } from '../utils/validate';
import {
  createDepartment,
  deleteDepartment,
  getDepartment,
  listDepartments,
  updateDepartment,
} from '../services/departments';

const router = Router();
const VIEW   = requirePermission('MASTER_DATA', 'master_data', 'view');
const CREATE = requirePermission('MASTER_DATA', 'master_data', 'create');
const EDIT   = requirePermission('MASTER_DATA', 'master_data', 'edit');
const DELETE = requirePermission('MASTER_DATA', 'master_data', 'delete');

const idParam = z.object({ id: z.string().uuid() });
const listQ = z.object({
  branchId: z.string().uuid().optional(),
  parentId: z.union([z.literal('root'), z.string().uuid()]).optional(),
  isActive: z.enum(['true', 'false']).optional(),
  page: z.coerce.number().int().positive().default(1),
  limit: z.coerce.number().int().min(1).max(200).default(50),
});
const createBody = z.object({
  code: z.string().min(1).max(50),
  name: z.string().min(1).max(200),
  description: z.string().max(1000).optional(),
  branchId: z.string().uuid().optional(),
  parentId: z.string().uuid().optional(),
});
const updateBody = z.object({
  name: z.string().min(1).max(200).optional(),
  description: z.string().max(1000).nullable().optional(),
  branchId: z.string().uuid().nullable().optional(),
  parentId: z.string().uuid().nullable().optional(),
  isActive: z.boolean().optional(),
});

router.get('/', requireInternal, VIEW, async (req, res, next) => {
  try {
    const q = parseQuery(req, listQ);
    sendSuccess(res, await listDepartments({
      branchId: q.branchId,
      // 'root' shorthand → top-level (parentId IS NULL)
      parentId: q.parentId === 'root' ? null : q.parentId,
      isActive: q.isActive === undefined ? undefined : q.isActive === 'true',
      page: q.page,
      limit: q.limit,
    }));
  } catch (err) { next(err); }
});

router.post('/', requireInternal, CREATE, async (req, res, next) => {
  try {
    sendSuccess(res, await createDepartment(parseBody(req, createBody)));
  } catch (err) { next(err); }
});

router.get('/:id', requireInternal, VIEW, async (req, res, next) => {
  try {
    sendSuccess(res, await getDepartment(parseParams(req, idParam).id));
  } catch (err) { next(err); }
});

router.put('/:id', requireInternal, EDIT, async (req, res, next) => {
  try {
    sendSuccess(res, await updateDepartment(
      parseParams(req, idParam).id,
      parseBody(req, updateBody),
    ));
  } catch (err) { next(err); }
});

router.delete('/:id', requireInternal, DELETE, async (req, res, next) => {
  try {
    await deleteDepartment(parseParams(req, idParam).id);
    sendSuccess(res, { ok: true });
  } catch (err) { next(err); }
});

export default router;
