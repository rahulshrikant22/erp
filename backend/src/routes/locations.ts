...
router.post('/', requireInternal, CREATE, async (req, res, next) => {
  try {
    sendSuccess(res, await createLocation(parseBody(req, createBody)));
  } catch (err) { next(err); }
});

router.get('/:id', requireInternal, VIEW, async (req, res, next) => {
  try {
    sendSuccess(res, await getLocation(parseParams(req, idParam).id));
  } catch (err) { next(err); }
});

router.put('/:id', requireInternal, EDIT, async (req, res, next) => {
  try {
    sendSuccess(res, await updateLocation(
...
router.delete('/:id', requireInternal, DELETE, async (req, res, next) => {
  try {
    await deleteLocation(parseParams(req, idParam).id);
