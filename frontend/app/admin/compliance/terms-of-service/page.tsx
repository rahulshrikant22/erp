...
export default function TermsOfServicePage(): React.ReactElement {
  const [versions, setVersions] = React.useState<TosVersion[] | null>(null);
  const [creating, setCreating] = React.useState(false);

  const fetch = React.useCallback(async () => {
    try {
      const r = await apiFetch<{ policies: TosVersion[] }>('/api/admin/compliance/terms-of-service');
  ...
  async function setActive(id: string) {
    try {
      await apiFetch(`/api/admin/compliance/terms-of-service/${id}/activate`, { method: 'POST' });
  ...
  const columns: ColumnDef<TosVersion>[] = [
  ...
  return (
    <>
      <PageHeader
        ...
        description="Manage versioned terms of service."
        actions={<Button size="sm" onClick={() => setCreating(true)}><Plus className="mr-1 h-4 w-4" /> New Version</Button>}
      />
      <DataTable columns={columns} data={versions ?? []} loading={versions === null} pageSize={10} emptyText="No versions." />

      <TosDialog open={creating} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); void fetch(); }} />
    </>
  );
}

function TosDialog({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    setSaving(true);
    try {
      await apiFetch('/api/admin/compliance/terms-of-service', {
        method: 'POST',
        body: { version, effectiveDate, content, isActive: false },
      });
      toast.success('ToS version created');
      onSaved();
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>New Terms Version</DialogTitle></DialogHeader>
